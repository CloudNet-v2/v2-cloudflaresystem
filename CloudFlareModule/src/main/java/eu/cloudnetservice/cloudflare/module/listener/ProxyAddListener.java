...
public class ProxyAddListener implements IEventListener<ProxyAddEvent> {
	...
	public void onCall(ProxyAddEvent event) {
		ModuleMain.getInstance().getCloud().getScheduler().runTaskAsync(() -> CloudFlareService.getInstance().addProxy(event.getProxyServer().getProcessMeta(), ModuleMain.getInstance().getCloudFlareDatabase()));
	}
}
